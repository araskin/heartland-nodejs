...
function HpsCreditService(hpsConfig, soapUri) {
     ...
     */
    var chargeWithToken =
        function chargeWithToken(amount, currency, token, cardHolder, requestMultiUseToken, memo, callback, InvoiceNbr) {
            console.log("executing Mojos Fork of Heartland NodeJS Library!");
            //this is the Mojo fork of the Heartland NodeJS library
            var schema = porticoSchema.requestType('CreditSale'),
                tx = { };
            ...
            if (hlp.defNn(cardHolder))  tx.CardHolderData = hlp.hydrateCardHolderData(cardHolder);
            if (hlp.defNn(memo))        tx.AdditionalTxnFields = { 'Description': memo };
            if (hlp.defNn(InvoiceNbr))  {
                console.log("Invoice Number is " + InvoiceNbr);
                tx.AdditionalTxnFields.InvoiceNbr = InvoiceNbr;
            }
            tx.AllowDup = 'N';
